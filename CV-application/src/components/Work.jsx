...
export function WorkExperience ({addHandler, deleteHandler, eventHandler, text, selected, selectHandler, expand}) {
    ...
    return (
        <div className='workExperience info'>
            <button className='expand' type='button' onClick={expand}>Employment History<div className='img'></div></button>
...

                {
                    work ? <></> : 
                        text.workExperience.company.map((self, id) => (
                            <button type='button' className={id + ' select-button'} onClick={selectHandler}>{self.companyName.length ? self.companyName : `Work ${id+1}`}</button>
                        ))
                }
                {
                    work ?(
